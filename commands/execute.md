# /execute — Orquestra a Etapa 12 (executar plano) respeitando o método

Você é um tech lead conduzindo a execução de uma fase. Este command é o **espelho de saída** do `/plan-phase`: assim como `/plan-phase` não deixa *começar* sem os 10 pré-requisitos, o `/execute` não deixa *fechar* uma task sem verificação.
...
- Para executar uma fase do roadmap que já tem plano aprovado pelo `@architect` (saída da Etapa 11 / `/plan-phase`).

**NÃO use** para:

- Gerar o plano → `/plan-phase`.
...
|---|---|---|---|
| 1 | Existe plano aprovado em `docs/superpowers/specs/` | `ls docs/superpowers/specs/*-plan.md` retorna ≥1 e CLAUDE.md Seção 5 aponta pra ele | "Rode `/plan-phase <fase>` antes." |
...
| 4 | `rules.yaml` válido | `bash .claude/hooks/rules-violation.sh` roda sem erro fatal | "Rode `/adr` em modo refine pra popular `rules.yaml`." |
...
5. **Scope-creep:** se a task revelou capacidade fora da feature-spec, a skill `detecting-scope-creep` deve ativar — pause e decida (ampliar spec via `/feature-spec` ou cortar) antes de continuar.
...
3. Sugira `/retro <fase>`. Se a fase entregou feature mensurável, sugira também `/hypothesis-check`.
...
- ❌ Não substitui `superpowers:executing-plans` — o invoca/complementa (igual `/plan-phase` faz com `writing-plans`).
- ❌ Não decide arquitetura (isso é `@architect` via `/adr`).
