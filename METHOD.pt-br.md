...
- **Toda hipótese tem um veredito.** A métrica decisiva do PRD existe pra ser confrontada com o número real: ✅ confirmada · ❌ caiu · 🤔 inconclusiva. Hipótese que nunca recebe veredito não era falseável — era fé. (Fechado por `/hypothesis-check`.)
...
- **Toda métrica tem um número que dispara decisão (kill criteria / tripwire).** Alvo sem threshold de fracasso é dashboard, não instrumento de gestão. "Se adoção < X% em N dias, para e repensa" é o que transforma medição em ação. (Definido em `/metrics`.)
- **Premissa de negócio arriscada se testa antes de construir, não depois.** Risco técnico tem `/spike`. Risco de **valor** ("alguém quer isso?") e **viabilidade** ("sustenta um negócio?") têm `/assumption-test` — o teste mais barato que mata a premissa mais cara. (Os 4 riscos de Cagan: valor, usabilidade, viabilidade, factibilidade.)
...
A regra é simples: **se aparece decisão com impacto > 1 sprint, abre `/adr`. Não importa em que etapa você está.**
...
| Tentou escrever feature F1 e percebeu que precisava decidir stack antes | Falta ADR. Pause F1, abra `/adr`.                              |
| Escreveu 8 ADRs mas elas não cobrem nenhuma feature real               | Detalhou demais cedo demais. Volte a features.                   |
| Plano de execução cita biblioteca que nenhuma ADR mencionou           | ADR faltando. Abra `/adr` durante execução, depois continue. |
...
|---|---|---|
| Nova aposta no **mesmo motor/codebase**, reforça o produto, morre junto com ele se a tese-mãe cair | **Nova hipótese no PRD existente** | O `/prd` suporta "N hipóteses entrelaçadas com N sinais de falsificação". Contexto num lugar só. |
...
- **Jornadas são por persona**, não por hipótese. Cada persona real ganha sua `/user-journey`.
...
TaskFlow ganha **emissão de nota fiscal**. Mesma Marina, mas é outra dor (burocracia fiscal, ~3h/mês) que o PRD não mapeava. → roda `/user-journey Marina` pro novo fluxo fiscal, vira **hipótese H3 no mesmo PRD** (linha no Histórico registrando a expansão), reusa as ADRs existentes + talvez 1 nova (integração com API da prefeitura, ADR com escopo declarado). **Não** é PRD novo — é a mesma aposta crescendo.
...
| ------------------------ | ------------------------------------------------------------------ | ----------------------------------------------- |
| `/prd <ideia>`         | Início de novo produto/feature grande                             | `docs/prd/<slug>.md`                          |
| `/persona <nome>`      | Antes de qualquer feature focada em usuário                       | `docs/business-context/personas.md` (append)  |
| `/user-journey <persona>`   | Após persona, antes de feature                                    | `docs/business-context/jornada.md` (append)   |
| `/metrics`            | Após PRD + personas, antes de features                            | `docs/business-context/metricas.md`           |
| `/adr <decisão>`      | Quando se identifica decisão estrutural                           | `docs/decisions/ADR-NNN-<slug>.md`            |
| `/feature-spec <nome>` | Antes de implementar feature                                       | `docs/features/F<N>-<slug>.md`                |
| `/feature-breakdown`   | Quando há 3+ features candidatas                                  | `docs/features/README.md` (binário MUST/OUT) |
| `/phase-roadmap`              | Após breakdown — distribuir todas as features em fases por tempo | `docs/roadmap/fases-overview.md`              |

### Design (entre planejamento e execução)

| Comando      | Quando usar                                      | O que gera                                               |
| ------------ | ------------------------------------------------ | -------------------------------------------------------- |
| `/screens` | Produto tem UI significativa (entre Etapa 8 e 9) | `docs/design/inventory.md` + `flow.md` + `briefs/` |
| `/extract-pattern <path>` | Destilar o padrão de um repo (starter/legado) em ADRs + doc carregável (envolve `@pattern-mapper`) | `docs/decisions/ADR-*` + `docs/technical-context/code-pattern-<scope>.md` |

### Desenvolvimento (operar dentro de uma fase)

| Comando                        | Quando usar                                                                                                    | O que gera                                                    |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `/plan-phase <fase>`          | Gerar plano de execução blindado (envolve `superpowers:writing-plans` + revisão `@architect`)           | `docs/superpowers/specs/<data>-fase-N-plan.md`              |
| `/execute <fase>`           | Executar a fase com plano aprovado (envolve `superpowers:executing-plans` + gate de pronto + `@qa-tester`) | código + plano marcado `running`→done                     |
| `/spike <pergunta>`          | Validar risco técnico antes de comprometer                                                                    | `docs/spikes/<data>-<slug>.md`                              |
| `/phase-scope <fase>`        | Decidir o que cabe na fase com orçamento fixo                                                                 | `docs/roadmap/fase-N-scope.md` (MoSCoW completo)            |
| `/triage <contexto>`         | Priorizar backlog de bugs/débitos                                                                             | `docs/triage/<data>-<contexto>.md` (MoSCoW completo)        |
| `/todo <add\|sync\|list\|done>` | Registrar e controlar pendências que não bloqueiam                                                           | `docs/backlog/todos.md` (ordenado por impacto no andamento) |
| `/refactor <alvo>`           | Guia refactor disciplinado                                                                                     | `docs/refactors/<data>-<slug>.md`                           |
| `/retro [fase]`              | Retrospectiva ao fim de fase                                                                                   | `docs/roadmap/fase-N-retro.md`                              |

### Navegação (transversal — em qualquer etapa)

| Comando         | Quando usar                                                     | O que gera                                               |
| --------------- | --------------------------------------------------------------- | -------------------------------------------------------- |
| `/status [N]` | Saber em que etapa do fluxo você está e qual o próximo passo | Diagnóstico em tela (lê `.claude/progress-map.yaml`) |

### Validação de produto (Measure-Learn — fecha o loop)

| Comando                           | Quando usar                                                              | O que gera                                                               |
| --------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------ |
| `/assumption-test <premissa>`   | **Antes** de construir: risco de valor/viabilidade não testado    | `docs/assumptions/<data>-<slug>.md`                                    |
| `/hypothesis-check [hipótese]` | **Depois** de construir: confrontar hipótese do PRD com dado real | `docs/hypothesis-checks/<data>-<slug>.md` + linha no Histórico do PRD |

> Estes dois materializam o princípio **"Evidência reabre decisão"** (ver "O loop fecha"). `/assumption-test` mata a aposta errada antes do custo; `/hypothesis-check` emite o veredito (✅/❌/🤔) e dispara ação — incluindo **sunset** de feature que não moveu a métrica.

### Quando usar MoSCoW binário vs completo

- **Binário (MUST / OUT)** em `/feature-breakdown`: pra decidir o que entra no **produto**.
- **MoSCoW completo (MUST / SHOULD / COULD / WON'T)** em `/phase-scope` e `/triage`: pra **operar** dentro do produto com orçamento limitado.

### `/todo` vs `/triage` (não confundir)

- **`/triage`** prioriza uma **rodada** de trabalho em MoSCoW — é uma decisão pontual ("o que ataco agora"). Gera um snapshot datado.
- **`/todo`** é o **registro persistente** de pendências que não bloqueiam, ordenado por impacto no andamento (🔴 bloqueia fase / 🟡 importa / ⚪ nice-to-have). Vive em `docs/backlog/todos.md` e nasce colado ao contexto via marcador inline `<!-- TODO[data][impacto]: ... -->` no próprio doc, reconciliado por `/todo sync`.
- Os dois conversam: os **WON'T** de um `/triage` são candidatos naturais a virar entradas no `/todo` pra não se perderem.
...
- **`@pattern-mapper`** — varre um repo e devolve o mapa do padrão (4 eixos, com força de evidência). Insumo do `/extract-pattern`. Não escreve ADR/doc.
...
- `verification-before-completion` — **é o gate de pronto do Princípio 4**: nenhuma task fecha (`[x]`) sem "teste passa + hook não bloqueia". Amarrada no Passo 3 do `/execute`. (Métrica observável, o 3º critério, é validada no fim de fase via `@qa-tester` / `/hypothesis-check`.)
...
| **`.claude/rules.yaml`**             | **Projeto**    | **`/adr`** (cada bloco nasce junto da ADR correspondente) |
...
| `docs/decisions/ADR-NNN-*.md`              | Projeto              | `/adr` (cada arquivo nasce de uma decisão)                     |
...
**Princípio:** ADR e seu bloco em `rules.yaml` são **gêmeos**. Nascem juntos via `/adr`. Vivem juntos. Se uma ADR vira `superseded`, seu bloco no `rules.yaml` é removido (ou atualizado para a ADR que a substituiu).

> ⚠️ **Anti-padrão observado na v0.1:** popular `rules.yaml` manualmente fora do fluxo `/adr` é débito técnico — o YAML perde a amarração com a ADR. O `/adr` v0.2 escreve direto no `rules.yaml` com confirmação rápida do usuário, evitando esse desvio.
...
- **MCP como fonte espelhada** (Jira recebe as tarefas) → **decisão estrutural: exige `/adr`**, porque muda onde a verdade espelha e pode aposentar fluxo nativo. O `/adr` escreve o bloco `integrations:` no `rules.yaml` (gêmeo da ADR).
